...
const SettingsPage = () => {
  ...
  if (!user) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#f4f7f2] text-sm font-medium text-[#27352f]">
        Loading account
      </main>
    )
  }
  ...
  return (
    <main className="min-h-screen bg-[#f4f7f2] px-5 py-10 text-[#1f2d28]">
          ...
          <section className="space-y-6">
            ...
            <div className="rounded-lg border border-[#d8dfd4] bg-white p-6 shadow-sm">
              ...
              <p className="mt-4 text-sm text-[#50635a]">
                Recovery codes are backup second-factor codes. If you lose
                access to your authenticator app, you can still use one of them
              ...
              {user.twoFactorSetupPending && !visibleSetup ? (
                <div className="mt-6 rounded-md border border-[#e1e6de] bg-[#fbfcfa] p-4 text-sm text-[#50635a]">
                  A setup is pending, but the QR code and recovery codes were
                  only shown when that setup started. If you need them again,
                  use <strong>Restart setup</strong> to generate a fresh secret
