...
const PlayerGrid = ({ slots }) => {
  ...
  return (
    <div className={`mb-11 grid gap-x-4 gap-y-3 ${layout}`}>
      {slots.map((slot, index) =>
        slot ? (
          <div key={slot.key} className="flex flex-col items-center gap-2">
            ...
            <span className="max-w-[96px] truncate text-sm font-bold text-[#2A1A08]">
              {slot.name}
            </span>
          </div>
        ) : (
          ...
          >
            ...
            />
            <span className="text-sm font-medium text-[#2A1A08]/40">
              Waiting
            </span>
          </div>
        ),
